...
import cn.seiua.skymatrix.config.option.WaypointSelect;
...
public class UIWaypoint extends UI {
    ...
    private OptionInfo<WaypointSelect> optionInfo;
    private float o = 0;

    public UIWaypoint(OptionInfo<WaypointSelect> optionInfo) {
...
//            tempdis = false;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String icon = Icons.LOCATION;
        ClickGui.iconfontRenderer22.drawString(matrixStack, getX() - 77, getY(), icon);
...
//
        String v = "none";
        if (optionInfo.getTarget().getValue() != null) {
            v = optionInfo.getTarget().getValue();
        }
        ...
        if (isInBox()) {
            ...
            ClickGui.iconfontRenderer22.centeredV();
            ClickGui.iconfontRenderer22.setColor(Theme.getInstance().THEME_UI_SELECTED.geColor());
            ClickGui.iconfontRenderer22.drawString(matrixStack, getX() + 77, getY(), Icons.LOOP);
            ...
            ClickGui.iconfontRenderer20.drawString(matrixStack, getX() + 77, getY(), Icons.LOOP);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0) {
            if (isInBox()) {
                this.optionInfo.getTarget().next();
